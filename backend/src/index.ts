...
app.get("/api/mount", async(req, res) => {
  res.json(data)
})
...
/*   await sleep(2);
  if (Math.random() > 0.95) return res.sendStatus(500);
  await sleep(2); */

  if ("includes" in query)
    return res.json(
      data.filter((hotel) => hotel.name.includes(query.includes))
    );
  return res.json(
    data.filter(
      (hotel) =>
        hotel.pricePerNightInUSD >= query.min &&
        hotel.pricePerNightInUSD <= query.max
    )
  );
});
